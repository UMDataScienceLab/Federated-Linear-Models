...
setwd("Wind/")
FD001 <- read.csv("OWEZData.csv", head=TRUE)
#View(FD001)
...
for(i in 1:K){
  ...
  train_size <- round(data_size*0.6)
  test_size <- data_size - train_size
  
  train_index <- sort(sample(c(1:data_size), size = train_size, replace = F))
  train_data <- Data[train_index,]
  test_index <- setdiff(c(1:data_size),train_index)
  test_data <- Data[test_index,]
  
  train_info_x <- range_normal(train_data[,1])
  train_info_y <- range_normal(train_data[,2])
  train_data[,1] <- train_info_x[[1]] 
  train_data[,2] <- train_info_y[[1]] 
  ...
  y <- append(y, list(train_data[,2]))
  
  
  ###  test
  
  test_info_x <- range_normal(test_data[,1])
  test_info_y <- range_normal(test_data[,2])
  test_data[,1] <- test_info_x[[1]] 
  test_data[,2] <- test_info_y[[1]] 
  ...
  y_test <- append(y_test, list(test_data[,2]))
...
# plot(y[[1]]~t(x[[1]])[,2])
# lines(t(x[[1]])%*%Theta0[,1]~t(x[[1]])[,2], col="red")
# 
